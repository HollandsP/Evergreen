...
def test_ffmpeg_subprocess_management():
    """Test FFmpeg subprocess resource management."""
    
    print("\n🔍 Testing FFmpeg subprocess management...")
    
    try:
        from src.services.ffmpeg_service import FFmpegService
        service = FFmpegService()
    except RuntimeError:
        print("   ⚠️ FFmpeg not available, skipping subprocess tests")
        return None
    
    # Monitor subprocess creation and cleanup
    initial_processes = len([p for p in psutil.process_iter(['name']) if 'ffmpeg' in p.info['name'].lower()])
    ...
    with tempfile.NamedTemporaryFile(suffix='.mp4', delete=False) as temp_input:
        try:
            ...
            final_processes = len([p for p in psutil.process_iter(['name']) if 'ffmpeg' in p.info['name'].lower()])
            
            process_growth = final_processes - initial_processes
            
            print(f"   Operations completed: {len(operations)}")
            print(f"   Process growth: {process_growth}")
            
            if process_growth <= 1:  # Allow for one lingering process
                print("   ✅ Subprocess management looks good")
                return True
            else:
                print(f"   ❌ Process leak detected: {process_growth} extra processes")
                return False
        ...
        print(f"   ❌ Error testing subprocess management: {e}")
        return False
...
def main():
    ...
    results = {
        ...
        'ffmpeg_subprocesses': test_ffmpeg_subprocess_management(),
